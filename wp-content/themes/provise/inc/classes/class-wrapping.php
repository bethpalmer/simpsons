 ...
 */
add_filter( 'template_include', array( 'Provise_Wrapping', 'wrap' ), 99 );
add_filter( 'provise_wrap_base', 'provise_wrap_base_cpts' );
 ...
 */
function provise_template_path() {
	return Provise_Wrapping::$main_template;
}
 ...
 */
function provise_template_base() {
	return Provise_Wrapping::$base;
}

if ( ! class_exists( 'Provise_Wrapping' ) ) {
	...
	class Provise_Wrapping {
		...
		/**
		 * Initialize new instance of Provise_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'provise_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Provise_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function provise_wrap_base_cpts( $templates ) {

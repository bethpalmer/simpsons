...
if ( ! class_exists( 'Provise_Widget_Area' ) ) {

	class Provise_Widget_Area {
		...
		function __construct() {
			...
			add_action( 'provise_render_widget_area', array( $this, 'render' ) );
		}
		...
		public function render( $area_id ) {
			...
			$area_id        = apply_filters( 'provise_rendering_current_widget_area', $area_id );
			...
			$classes = apply_filters( 'provise_widget_area_classes', $classes, $area_id );
	...
	function provise_widget_area() {
		return Provise_Widget_Area::get_instance();
	}

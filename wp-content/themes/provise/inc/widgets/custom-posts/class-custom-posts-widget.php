 ...
 * @package Provise
 */
if ( ! class_exists( 'Provise_Custom_Posts_Widget' ) ) {
	class Provise_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		/**
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'provise' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'provise' );
			$this->widget_id          = apply_filters( 'provise_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'provise_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = provise_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'provise' ),
					'label' => esc_html__( 'Title', 'provise' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
					'options' => array(
						'category_name' => array(
							'label' => esc_html__( 'Category', 'provise' ),
							'slave' => 'terms_type_post_category',
						),
						'tag' => array(
							'label' => esc_html__( 'Tag', 'provise' ),
							'slave' => 'terms_type_post_tag',
						),
						'post_format' => array(
							'label' => esc_html__( 'Post Format', 'provise' ),
							'slave' => 'terms_type_post_format',
						),
					),
					'label' => esc_html__( 'Choose taxonomy type', 'provise' ),
				),
				'category_name' => array(
					'type'             => 'select',
					'size'             => 1,
					...
					'options'          => false,
					'label'            => esc_html__( 'Select category', 'provise' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select category', 'provise' ),
				...
				'tag' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'provise' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select tags', 'provise' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'provise' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'provise' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'provise' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'provise' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'provise' ),
				),
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'provise' ),
				),
				'meta_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'          => esc_html__( 'Date', 'provise' ),
						'author'        => esc_html__( 'Author', 'provise' ),
						'comment_count' => esc_html__( 'Comment count', 'provise' ),
						'category'      => esc_html__( 'Category', 'provise' ),
						'post_tag'      => esc_html__( 'Tag', 'provise' ),
						'more_button'   => esc_html__( 'More Button', 'provise' ),
					),
					'label' > esc_html__( 'Display post meta data', 'provise' ),
				),
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'provise' ),
				),
			);
			parent::__construct();
		}
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'        => 'provise-thumb-m',
						'mobile_size' => 'provise-thumb-s',
					...
					$count = $this->utility->meta_data->get_comment_count( array(
						...
						'sufix'   => _n_noop( '%s comment', '%s comments', 'provise' ),
					) );
	...
	add_action( 'widgets_init', 'provise_register_custom_posts_widget' );
	function provise_register_custom_posts_widget() {
		register_widget( 'Provise_Custom_Posts_Widget' );
	}
}